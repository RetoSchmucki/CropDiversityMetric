...
To compute each of these IBR curves, we can use the rarefaction function implemented in the R package mobr. This function estimates the IBR curves from an abundance matrix, here described as the number of field of year sown for each crop. Below, we wrote two R-functions to estimate the IBR curve along the temporal dimension respectively. In these functions, we first need to generate a matrix with the total number of occurrence for each crop across the different fields (parcel) in a given year or across the different years in a given field. A third function can be written to generate IBR curves that consider the crop diversity observed in the overall landscape, combining both the spatial and temporal dimension of the cropping system.
...
When crop sequence available are limited in time, we can extrapolate the diversity metric beyond the number of cropping event available. It is suggested that the extrapolated value are acceptable up to twice the length of the sequence available. Although this has been demonstrated to be acceptable for biological diversity, it is less clear if this is also valid for crop diversity sown in rotation systems. To assess the validity of the extrapolated diversity, we used real crop sequences grown across 500 landscapes (5x5km) distributed across Denmark and France. In each country, we extracted the information of 8-year crop sequences grown in 28,092 and 35,079 arable fields, in Denmark and France respectively. From these sequences, we computed the temporal component of crop diversity across the 500 landscapes, contrasting the diversity values estimated from the entire 8-year crop sequences with the values obtained when extrapolating the crop diversity of height years from 4-year crop sequences.

The data we used are a subset of real crop data, that we extracted for 500 randomly sampled landscapes that the location have been censured to the large geographic region.   

Warning: The crop_seq_data.csv data set is waiting for agreement before release.
\vspace{0.5cm}
...
From these sequences, we computed the average diversity estimated in arable field within each of the 500 landscapes. We computed the 8-year diversity metric from both the height year and the four year truncated sequences. 

Warning: the computing the indices across all field and landscape is computer intensive and for this example restricted the code below to five landscapes
